...
type
  Tfdataarincomingpayment = class(TForm)
    Panel1: TPanel;
    op: TOpenDialog;
    Panel2: TPanel;
    sEdit1: TsEdit;
...
var
  fdataarincomingpayment: Tfdataarincomingpayment;
...
function Tfdataarincomingpayment.convertMonthNametoIndex(bulan: string): string;
begin
...
procedure Tfdataarincomingpayment.Button1Click(Sender: TObject);
var idcustomer,tahun,bulan:string;
begin
  idcustomer:=qdatacustomer_code_sap.Value;
  tahun:=formatdatetime('yyyy',qdatatanggal.Value);
  bulan:=formatdatetime('m',qdatatanggal.Value);

  fdetailuangmasuk.qdata.SQL.Clear;
  fdetailuangmasuk.qdata.SQL.Add('SELECT DATE(dk.tanggal_transfer) AS tanggal,c.customer_code_sap, '+
  'c.`nama`,dk.jumlah_uang AS jml, '+
  'dk.no '+
  ...
  'WHERE c.customer_code_sap='''+idcustomer+''' and YEAR(dk.tanggal_transfer) = '''+tahun+''' AND MONTH(dk.tanggal_transfer) = '''+bulan+''' AND  '+
  '((LCASE(TRIM(SUBSTRING_INDEX(dk.bank, ''#'', -1))) = ''bca'') OR       '+
  '(LCASE(TRIM(SUBSTRING_INDEX(dk.bank, ''#'', -1))) = ''mandiri'') OR    '+
  '(LCASE(TRIM(SUBSTRING_INDEX(dk.bank, ''#'', -1))) = ''bri'')) ');
  fdetailuangmasuk.qdata.Active:=True;

  fdetailuangmasuk.qdata2.SQL.Clear;
  fdetailuangmasuk.qdata2.SQL.Add('SELECT tanggal,customer_code_sap,nama,nominal AS jml '+
  ' FROM `a_sap_arinvoice` WHERE customer_code_sap='''+idcustomer+''' and sts=''INCOMING PAYMENT'' AND SUBSTR(customer_code_sap,1,1) = ''C'' '+
  ' AND YEAR(tanggal) = '''+tahun+''' AND MONTH(tanggal) = '''+bulan+''' ');
  fdetailuangmasuk.qdata2.Active:=True;

  fdetailuangmasuk.ShowModal;
end;

procedure Tfdataarincomingpayment.Button2Click(Sender: TObject);
var
  idcustomer,idkonfirmasi,urutan,refno,periode:String;
begin
  ...
  while not qdata.eof do begin
    if qdataselisih.AsFloat > 0 then begin
      idcustomer:=qdatacustomer_code_sap.Value;
      tabel.q1.SQL.Clear;
      tabel.q1.SQL.Add('SELECT dk.no '+
      ' FROM `detail_konfirmasipembayaran` dk   '+
      ' JOIN `uang_customer` uc ON uc.`id_konfirmasi`=dk.`no`   '+
      ' JOIN customer c ON c.`id_customer`=uc.`id_customer`   '+
      ' WHERE c.customer_code_sap='''+idcustomer+''' and DATE_FORMAT(dk.tanggal_transfer, ''%Y-%m'') ='''+periode+''' AND  '+
      ' ((LCASE(TRIM(SUBSTRING_INDEX(dk.bank, ''#'', -1))) = ''bca'') OR       '+
      ' (LCASE(TRIM(SUBSTRING_INDEX(dk.bank,  ''#'', -1))) = ''mandiri'') OR    '+
      ' (LCASE(TRIM(SUBSTRING_INDEX(dk.bank,  ''#'', -1))) = ''bri'')) ');
      ...
      while not tabel.q1.Eof do begin
        ...
        ' WHERE tabel=''Konfirmasi_pembayaran'' and no_transaksi='''+idkonfirmasi+''' ');
        ...
        if tabel.q2.RecordCount > 0 then begin
          ...
          if tabel.q2.RecordCount > 0 then begin
            ...
            tabel.q2.SQL.Clear;
            tabel.q2.SQL.Add('CALL sinkronisasi_sap('''+ idkonfirmasi +''','''+ refno +''', ''konfirmasi_pembayaran'',''/saptemp/sinkterimauang'',@rest_sinkronisasisap)  ');
            tabel.q2.ExecSQL;
          end;
        end;
      tabel.q1.Next;
...
procedure Tfdataarincomingpayment.Button3Click(Sender: TObject);
var periode:String;
begin
periode:=combobox5.Text+'-'+convertMonthNametoIndex(ComboBox6.Text);

qdata.SQL.Clear;
qdata.SQL.Add('SELECT dt.tanggal,dt.customer_code_sap,dt.nama,dt.jml AS jmlknitto,IFNULL(ds.jml,0) AS jmlsap,(dt.jml - IFNULL(ds.jml,0)) AS selisih '+
'FROM (SELECT DATE(dk.tanggal_transfer) AS tanggal,c.customer_code_sap,c.`nama`,SUM(dk.jumlah_uang) AS jml '+
'FROM `detail_konfirmasipembayaran` dk  '+
'JOIN `uang_customer` uc ON uc.`id_konfirmasi`=dk.`no`    '+
'JOIN customer c ON c.`id_customer`=uc.`id_customer`       '+
...
'LEFT JOIN  '+
'(SELECT tanggal,customer_code_sap,nama,SUM(nominal) AS jml  '+
'FROM `a_sap_arinvoice` WHERE sts=''INCOMING PAYMENT'' AND SUBSTR(customer_code_sap,1,1) = ''C''  '+
...
procedure Tfdataarincomingpayment.ComboBox5DropDown(Sender: TObject);
...
procedure Tfdataarincomingpayment.cxButton1Click(Sender: TObject);
begin
  Button3Click(nil);
...
procedure Tfdataarincomingpayment.cxButton2Click(Sender: TObject);
var
...
//  end;
  tabel.q1.SQL.Clear;
  tabel.q1.SQL.Add('delete from `a_sap_arinvoice` where sts=''INCOMING PAYMENT'' ');
  tabel.q1.ExecSQL;
...
//        if tabel.q1.RecordCount = 0 then begin
          tabel.q1.SQL.Clear;
          tabel.q1.SQL.Add('insert into a_sap_arinvoice values(null,'''+ tanggal +''','''+ customer_code_sap +''','''+ nama +''','''+ nominal +''','''+ ref_no +''','''+ cabang +''','''+ no_transaksi +''',''INCOMING PAYMENT'') ');
          tabel.q1.ExecSQL;
//        end else begin
//          tabel.q1.SQL.Clear;
...
procedure Tfdataarincomingpayment.FormShow(Sender: TObject);
begin
  sedit1.text:='';
...
procedure Tfdataarincomingpayment.SpeedButton1Click(Sender: TObject);
begin
  if op.Execute then
