...
uses
  ...
  Vcl.StdCtrls, cxButtons, sEdit, Vcl.Buttons, Vcl.ExtCtrls,comobj, dxSkinBlack,
...
function Tfuangedc.convertMonthNametoIndex(bulan: string): string;
begin
...
procedure Tfuangedc.Button2Click(Sender: TObject);
var
...
procedure Tfuangedc.Button3Click(Sender: TObject);
var periode:String;
begin
periode:=combobox5.Text+'-'+convertMonthNametoIndex(ComboBox6.Text);
...
' WHERE (jenis_pembayaran=''CC'' OR jenis_pembayaran=''DEBIT'') AND SUBSTR(nt.tanggal,1,7)='''+periode+''' ');
qdata.Active:=True;
...
procedure Tfuangedc.ComboBox5DropDown(Sender: TObject);
...
procedure Tfuangedc.cxButton2Click(Sender: TObject);
var
...
//  end;
  tabel.q1.SQL.Clear;
  tabel.q1.SQL.Add('delete from `a_sap_arinvoice` where sts=''EDC'' ');
  tabel.q1.ExecSQL;
...
//        if tabel.q1.RecordCount = 0 then begin
          tabel.q1.SQL.Clear;
          tabel.q1.SQL.Add('insert into a_sap_arinvoice values(null,'''+ tanggal +''','''+ customer_code_sap +''','''+ nama +''','''+ nominal +''','''+ ref_no +''','''+ cabang +''','''+ no_transaksi +''',''EDC'') ');
          tabel.q1.ExecSQL;
...
procedure Tfuangedc.SpeedButton1Click(Sender: TObject);
begin
  if op.Execute then
