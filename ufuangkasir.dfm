object fuangkasir: Tfuangkasir
  ...
  object Panel1: TPanel
    ...
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Incoming Payment Kasir'
    Color = clHotLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -19
  ...
  object qdata: TUniQuery
    ...
    SQL.Strings = (
        ...
        'SELECT nt.no AS nokasir,nt.tanggal,customer_code_sap,nama,no_ord' +
        'er,sd.`ref_no`,(nt.`jumlah` - IFNULL(dt.nominal,0)) AS jumlah,'
      
        'IFNULL(ks.nominal,0) AS nominalsap,(nt.`jumlah` - IFNULL(dt.nomi' +
        'nal,0))-IFNULL(ks.nominal,0) AS selisih'
      'FROM n_temp_kasirdetail nt '
      
        'LEFT JOIN (SELECT jk.`id_kasir`,ak.`nominal` FROM `junct_kembali' +
        'ankasir` jk'
      
        'JOIN `a_kembalian` ak ON jk.`id_kembalian`=ak.`no`) dt ON dt.id_' +
        'kasir=nt.`no`'
      
        'LEFT JOIN (SELECT * FROM sinkronisasi_data WHERE `tabel`='#39'terima' +
        'uang_kasir'#39') sd ON nt.no=sd.no_transaksi '
      'LEFT JOIN `a_sap_arinvoice` ks ON ks.`ref_no`=sd.`ref_no`'
      
        'WHERE jenis_pembayaran='#39'CASH'#39' AND SUBSTR(nt.tanggal,1,7)='#39'2021-1' +
        '2'#39' AND jumlah !=0 '
      '')
    Left = 504
    Top = 152
